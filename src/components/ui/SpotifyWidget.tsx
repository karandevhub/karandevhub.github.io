...
export default function SpotifyWidget() {
  ...
  return (
    <a
      ...
      aria-label="Spotify"
      className="group inline-flex items-center gap-2.5 rounded-xl border border-border-subtle bg-bg-secondary/60 px-2.5 py-1.5 transition-all duration-300 hover:border-[#1DB954]/40 hover:bg-[#1DB954]/5 hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1DB954]/50"
    >
        ...
        <div className="relative flex flex-col min-w-0">
          <div className="flex items-center gap-4 whitespace-nowrap">
            <div className="flex flex-col min-w-0">
              <span className="text-[13px] font-semibold text-text-primary leading-tight truncate mb-0.5">
                {track.title || "Spotify"}
              </span>
              {track.artist && (
                <span className="font-mono text-[9px] uppercase tracking-widest text-text-muted truncate">
                  {track.artist}
                </span>
              )}
            </div>
